class Storey::Hstore
  ...
  def install
    ...
    if e.message =~ /could not open extension control file/
      fail Storey::StoreyError, "Oops! Looks like the Hstore extension is not installed. Please install it for your OS first."
    end
    fail e
  end
  ...
  def ensure_hstore_is_persistent
    unless Storey.persistent_schemas.include?('hstore')
      fail Storey::StoreyError, 'You are attempting to install hstore data type, but the hstore schema (where the data type will be installed) is not one of the persistent schemas. Please add hstore to the list of persistent schemas.'
    end
  end
